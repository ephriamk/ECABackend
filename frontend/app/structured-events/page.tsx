...
export default function StructuredEventsPage() {
  ...
  // Fetch events from structured_events.db for a month
  const loadEvents = () => {
    setLoading(true);
    const { start, end } = getMonthRange(month);
    const url = `http://localhost:8000/api/api-events/structured-events/db?start_date=${start}&end_date=${end}`;
    fetch(url)
      .then(res => res.json())
      .then(json => {
  ...
  const fetchLatest = () => {
    setFetching(true);
    setError(null);
    fetch("http://localhost:8000/api/api-events/structured-events/fetch", { method: "POST" })
      .then(res => res.json())
      .then(() => {
        setFetching(false);
        loadEvents();
  ...
  // Get all unique columns, but hide none for raw view
  ...
  return (
    <div className="max-w-7xl mx-auto p-8">
      <h1 className="text-2xl font-bold mb-6">Structured Events (Raw Data)</h1>
